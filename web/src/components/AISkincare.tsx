...
const extractURLsFromText = (text: string): Source[] => {
  ...
  const urlRegex =
    /(?:\[|\(|\bsource:|\bfrom:|\bhttps?:\/\/)[^\s[\]()<>"']+(?:(?:\([\w\d]+\)|[\w\d]+\.[!\w\d]+)+(?:[/\w\d#%&()=?:;,.@+~-]*[/\w\d#%&()=?:~-])?)/gi;
  ...
  for (const match of matches) {
    try {
      ...
      console.error("Error processing URL from text:", match, error);
      // Not a valid URL, skip it
    }
  }
...
export function AISkincare() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hi there! I'm your skincare AI assistant. I can help answer questions about your skincare routine, product recommendations, or learn about ingredients. I can also search the web for the latest skincare information and create images to help visualize concepts.",
    },
  ]);
  ...
  useEffect(() => {
    const setContainerHeight = () => {
      if (chatContainerRef.current && window.innerWidth < 768) {
        ...
        const headerOffset = 180; // Approximate header height + tabs + search info
        chatContainerRef.current.style.height = `calc(${viewportHeight}px - ${headerOffset}px)`;
      } else if (chatContainerRef.current) {
        chatContainerRef.current.style.height = '600px'; // Increased default height for desktop
      }
    };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const userContext = `
        ...
        ${userData.products
          .map((p) => {
            const ingredients = p.ingredients
              ? `Contains: ${p.ingredients.slice(0, 5).join(", ")}${
        ...
        - The user's main focus areas appear to be: ${userData.skinConcerns
          .slice(0, 3)
          .join(", ")}
      `;
      ...
      if ((await result)?.providerMetadata?.google) {
        const googleMetadata = (await result).providerMetadata
          ?.google as GoogleMetadata;
        // Extract citation metadata
        if (googleMetadata?.citationMetadata?.citations) {
          assistantMessage.sources =
            googleMetadata.citationMetadata.citations.map((citation) => ({
              uri: citation.uri,
        ...
        if (googleMetadata.groundingMetadata) {
          assistantMessage.searchMetadata = {
            webSearchQueries:
              googleMetadata.groundingMetadata.webSearchQueries || [],
          };
        }
        ...
        if ((await result).sources && (await result).sources.length > 0) {
          ...
          assistantMessage.sources = (await result).sources
            .map((source) => {
      ...
      if ((await result).sources) {
        try {
          ...
          const sourcesFromResult = Array.isArray((await result).sources)
            ? (await result).sources.map((source) => {
      ...
      try {
        ...
        const googleMetadataStandard = (await result).providerMetadata
          ?.google as GoogleMetadata | undefined;
        const resultWithExperimental = result as any; // Use any for accessing non-standard properties
        const googleMetadataExperimental = resultWithExperimental
          ?.experimental_providerMetadata?.google as GoogleMetadata | undefined;

        const renderedContent =
          googleMetadataStandard?.groundingMetadata?.searchEntryPoint
            ?.renderedContent ||
          googleMetadataExperimental?.groundingMetadata?.searchEntryPoint
            ?.renderedContent;
        ...
        const processedFiles = await Promise.all(
          (
            ...
            if ("base64Data" in file && typeof file.base64Data === "string") {
              ...
              try {
                const base64 = file.base64Data.replace(
                  /^data:[^;]+;base64,/,
                  ""
                );
  ...
  const renderImageFromFile = (file: {
    data: Uint8Array;
    mimeType: string;
  }) => {
    ...
    const handleDownload = (e: React.MouseEvent) => {
      ...
      link.href = url;
      link.download = `skincare-image-${new Date().getTime()}.${
  ...
  const renderSources = (
    sources: Source[],
    searchMetadata?: { webSearchQueries?: string[] }
  ) => {
  ...
  return (
    <div className="min-h-screen bg-background pb-16 md:pb-8">
      ...
      <div className="container mx-auto px-4 py-2 md:py-8 max-w-7xl">
        {/* Hero Section - More compact on mobile */}
        <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-purple-500/40 via-purple-400/30 to-background border border-purple-500/40 dark:border-purple-700/30 p-3 md:p-8 mb-4 md:mb-6">
          <div className="absolute inset-0 bg-grid-pattern opacity-15"></div>
          <div className="relative flex flex-col md:flex-row md:items-center gap-2 md:gap-8">
            <div className="flex-1 space-y-2 md:space-y-3">
              <div className="inline-flex items-center gap-2 px-2 py-1 rounded-full bg-purple-500/30 text-purple-700 dark:text-purple-400 text-xs md:text-sm font-medium mb-1 md:mb-2 shadow-md backdrop-blur-sm border border-purple-500/40">
                <Bot className="h-3 w-3 md:h-4 md:w-4" />
                AI Skincare Assistant
              </div>
              <h1 className="text-2xl md:text-4xl font-bold tracking-tight mb-1 md:mb-2 text-purple-700 dark:text-purple-300">
                Your Personal Skincare AI
              </h1>
              <p className="text-muted-foreground max-w-xl backdrop-blur-sm bg-background/50 p-2 md:p-3 rounded-lg border border-purple-500/30 shadow-sm text-sm md:text-lg">
                Ask questions about your skincare routine, get personalized recommendations, or create visual guides.
              </p>
            </div>
        ...
        <div className="md:hidden">
          <Tabs defaultValue="chat" className="mb-3">
              ...
              <Card className="border shadow-md flex-1 flex flex-col overflow-hidden">
                <CardContent className="p-0 flex flex-col h-full">
                  ...
                  <div
                    ...
                    className="overflow-y-auto p-3 space-y-4 flex-1 scroll-smooth"
                  >
                          ...
                          {message.role === "assistant" ? (
                            <>
                              ...
                              {message.files?.map((file, fileIdx) => (
                                <div key={fileIdx}>{renderImageFromFile(file)}</div>
                              ))}
            ...
            <TabsContent value="image" className="mt-0">
              <Card className="border shadow-md flex-1 flex flex-col overflow-hidden">
                <CardContent className="p-0 flex flex-col h-full">
                  {/* Image Generation Messages Container */}
                  <div
                    ref={chatContainerRef}
                    className="overflow-y-auto p-3 space-y-4 flex-1 scroll-smooth"
                  >
                  ...
                  <div className="border-t p-3 mt-auto">
                    <div className="flex gap-2">
                      <Textarea
                        ...
                        onChange={(e) => setInput(e.target.value)}
                        onKeyDown={handleKeyDown}
                        placeholder="Describe the skincare image you want to create..."
                        className="min-h-[50px] resize-none"
                      />
                      <Button
                        onClick={handleSendMessage}
        ...
        <div className="hidden md:block">
          ...
          <Card className="border shadow-md flex-1 flex flex-col overflow-hidden">
            <CardContent className="p-0 flex flex-col h-full">
              ...
              <div
                ref={chatContainerRef}
                className="overflow-y-auto p-6 space-y-4 flex-1 scroll-smooth"
              >
                {messages.map((message, index) => (
                  <div
                    key={index}
